...
export default function NovaSaidaPage() {
  ...
  function handleChange(
    ...
    const field = event.target.name as keyof ExpenseForm
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message = getErrorMessage(err, 'Falha ao salvar saída')
      console.error('Erro ao salvar saída:', {
        message,
        fullError: err,
      })
      setError(message)
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-[rgba(26,10,8,0.07)]">
        <div className="max-w-4xl mx-auto px-4 lg:px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-2xl font-bold text-[#1A0A08]">Nova saída</h1>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-4xl mx-auto w-full px-4 lg:px-6 py-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <section className="rounded-[16px] border border-[rgba(26,10,8,0.07)] bg-white p-5 lg:p-6">
            <h2 className="mb-4 text-lg font-bold text-[#1A0A08]">Dados da saída</h2>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
            ...
            >
              ...
              <span>{isSubmitting ? 'Salvando...' : 'Salvar saída'}</span>
            </button>
          </div>
        </form>
      </main>
